...
In order to kick start your exploration of software, this four week programme will look at the following:
...
A very simple use of a regular expression would be to locate the same word spelled two different ways. For example the regular expression `organ[sz]e` matches both "organise" and "organize".
...
{Question}: So, what is `^[Oo]rgani.e$` going to match.
